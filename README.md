...
* connection.salesforce.EventIDStoredFilePath:
    ...
    ```
    Ex: salesforce object is /topic/reading__e then event id property name is "reading__e".
...
```
To subscribe to multiple platform events or push topics add the seperate inbound endpoit with corresponding sequences for push topics and platform events. Make sure to create resource in the config registry DB to for each object to store event id when replay option need to use.
```
